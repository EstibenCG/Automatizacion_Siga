...
public class editar_asignatura implements Task {

    private final DatosAsignatura datos;

    public editar_asignatura(DatosAsignatura datos) {
        this.datos = datos;
    }
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                Click.on(BTN_MODULO_ASIGNATURAS),
                Click.on(BTNEDITAR),
    ...
    public static editar_asignatura conDatos(DatosAsignatura datos) {
        return Instrumented.instanceOf(editar_asignatura.class).withProperties(datos);
    }
}